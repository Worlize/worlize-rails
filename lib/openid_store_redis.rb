...
module OpenID
  module Store
    class Redis < OpenID::Store::Interface
      ...
      def store_association(server_url, association)
        ...
        [nil, association.handle].each do |handle|
          ...
          @cache_client.setex(key, association.expires_in, serialized)
        end
      end
      ...
      def use_nonce(server_url, timestamp, salt)
        ...
        result = @cache_client.setex(nonce_key, expiry(Nonce.skew + 5), '')
      ...
      # Return the input value, since redis thinks in seconds rather than
      # in timestamps.
      def expiry(t)
        t.to_i
      end
